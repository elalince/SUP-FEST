  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    function initDropdowns(container) {
      container.querySelectorAll(".dropdown").forEach((dropdown) => {
        ...
        function updateSelectedItem(value, list2) {
          list2.querySelectorAll(".dropdown__item").forEach((item) => {
            item.classList.remove("selected");
          });
          const activeItem = Array.from(list2.children).find(
    ...
    function saveAndShowSummary() {
      try {
        ...
        `;
          if (shopSummaryList)
            shopSummaryList.appendChild(itemElement);
        });
